...
from cbank.cli.controllers import (
    ...
    detail_refunds_main)
from cbank.cli.exceptions import (
...
def current_user ():
    return User.fetch(current_username())

def project_allocations (project):
    return Session.query(Allocation).filter_by(project_id=project.id)
...
def be_admin ():
    ...
    cbank.config.set("cli", "admins", current_user)


def not_admin ():
    cbank.config.remove_option("cli", "admins")
...
class CbankTester (object):
    ...
    def setup (self):
        metadata.create_all()
        cbank.config.add_section("cli")
    
    def teardown (self):
        Session.remove()
        cbank.config.remove_section("cli")
        metadata.drop_all()
...
class TestDetailCharges (CbankTester):
    ...
    def setup (self):
        ...
        self._print_charges = cbank.cli.controllers.print_charges
        cbank.cli.controllers.print_charges = FakeFunc()
    
    def teardown (self):
        CbankTester.teardown(self)
        cbank.cli.controllers.print_charges = self._print_charges
    ...
    def test_user (self):
        ...
        args, kwargs = cbank.cli.controllers.print_charges.calls[0]
...
class TestDetailRefunds (CbankTester):
    ...
    def setup (self):
        CbankTester.setup(self)
        self._print_refunds = cbank.cli.controllers.print_refunds
        cbank.cli.controllers.print_refunds = FakeFunc()
    
    def teardown (self):
        CbankTester.teardown(self)
        cbank.cli.controllers.print_refunds = self._print_refunds
    ...
    def test_user (self):
        ...
        args, kwargs = cbank.cli.controllers.print_refunds.calls[0]
...
class TestMain (CbankTester):
    ...
    def setup (self):
        CbankTester.setup(self)
        self._list_main = cbank.cli.controllers.list_main
        self._new_main = cbank.cli.controllers.new_main
        self._detail_main = cbank.cli.controllers.detail_main
        self._edit_main = cbank.cli.controllers.edit_main
        cbank.cli.controllers.list_main = FakeFunc()
        cbank.cli.controllers.new_main = FakeFunc()
        cbank.cli.controllers.detail_main = FakeFunc()
        cbank.cli.controllers.edit_main = FakeFunc()
    
    def teardown (self):
        CbankTester.teardown(self)
        cbank.cli.controllers.list_main = self._list_main
        cbank.cli.controllers.new_main = self._new_main
        cbank.cli.controllers.detail_main = self._detail_main
        cbank.cli.controllers.edit_main = self._edit_main
    ...
    def test_list (self):
        ...
        cbank.cli.controllers.list_main.func = test_
        args = "list 1 2 3"
        run(main, args.split())
        assert cbank.cli.controllers.list_main.calls
    
    def test_new (self):
        def test_ ():
            assert sys.argv[0] == "main new", sys.argv
            assert sys.argv[1:] == args.split()[1:], sys.argv
        cbank.cli.controllers.new_main.func = test_
        args = "new 1 2 3"
        run(main, args.split())
        assert cbank.cli.controllers.new_main.calls
    
    def test_detail (self):
        def test_ ():
            assert sys.argv[0] == "main detail", sys.argv
            assert sys.argv[1:] == args.split()[1:], sys.argv
        cbank.cli.controllers.detail_main.func = test_
        args = "detail 1 2 3"
        run(main, args.split())
        assert cbank.cli.controllers.detail_main.calls
    
    def test_edit (self):
        def test_ ():
            assert sys.argv[0] == "main edit", sys.argv
            assert sys.argv[1:] == args.split()[1:], sys.argv
        cbank.cli.controllers.edit_main.func = test_
        args = "edit 1 2 3"
        run(main, args.split())
        assert cbank.cli.controllers.edit_main.calls
...
class TestNewMain (CbankTester):
    ...
    def setup (self):
        CbankTester.setup(self)
        be_admin()
        self._new_allocation_main = cbank.cli.controllers.new_allocation_main
        self._new_hold_main = cbank.cli.controllers.new_hold_main
        self._new_charge_main = cbank.cli.controllers.new_charge_main
        self._new_refund_main = cbank.cli.controllers.new_refund_main
        cbank.cli.controllers.new_allocation_main = FakeFunc()
        cbank.cli.controllers.new_hold_main = FakeFunc()
        cbank.cli.controllers.new_charge_main = FakeFunc()
        cbank.cli.controllers.new_refund_main = FakeFunc()
    
    def teardown (self):
        CbankTester.teardown(self)
        cbank.cli.controllers.new_allocation_main = self._new_allocation_main
        cbank.cli.controllers.new_hold_main = self._new_hold_main
        cbank.cli.controllers.new_charge_main = self._new_charge_main
        cbank.cli.controllers.new_refund_main = self._new_refund_main
    
    def test_exists_and_callable (self):
        assert hasattr(cbank.cli.controllers, "new_main"), "new_main does not exist"
        assert callable(cbank.cli.controllers.new_main), "new_main is not callable"
    ...
    def test_allocation (self):
        ...
        def test_ ():
            ...
            assert sys.argv[1:] == args.split()[1:], sys.argv
        cbank.cli.controllers.new_allocation_main.func = test_
        run(new_main, args.split())
        assert cbank.cli.controllers.new_allocation_main.calls
    ...
    def test_hold (self):
        ...
        def test_ ():
            ...
            assert sys.argv[1:] == args.split()[1:], sys.argv
        cbank.cli.controllers.new_hold_main.func = test_
        run(new_main, args.split())
        assert cbank.cli.controllers.new_hold_main.calls
    ...
    def test_charge (self):
        ...
        def test_ ():
            ...
            assert sys.argv[1:] == args.split()[1:], sys.argv
        cbank.cli.controllers.new_charge_main.func = test_
        run(new_main, args.split())
        assert cbank.cli.controllers.new_charge_main.calls
    ...
    def test_refund (self):
        ...
        def test_ ():
            ...
            assert sys.argv[1:] == args.split()[1:], sys.argv
        cbank.cli.controllers.new_refund_main.func = test_
        run(new_main, args.split())
        assert cbank.cli.controllers.new_refund_main.calls
...
class TestNewAllocationMain (CbankTester):
    ...
    def test_exists_and_callable (self):
        assert hasattr(cbank.cli.controllers, "new_allocation_main"), \
            "new_allocation_main does not exist"
        assert callable(cbank.cli.controllers.new_allocation_main), \
    ...
    def test_complete (self):
        ...
        code, stdout, stderr = run(
            cbank.cli.controllers.new_allocation_main, args.split())
        Session.remove()
    ...
    def test_with_defined_units (self):
        cbank.config.set("cli", "unit_factor", "1/2")
        project = Project.fetch("project1")
        resource = Resource.fetch("resource1")
        query = lambda: Session.query(Allocation).filter_by(
            project_id=project.id, resource_id=resource.id)
    ...
    def test_without_project (self):
        ...
        assert_equal(code, MissingArgument.exit_code)
    ...
    def test_with_configured_resource (self):
        cbank.config.set("cli", "resource", "resource1")
        project = Project.fetch("project1")
        resource = Resource.fetch("resource1")
        query = lambda: Session.query(Allocation).filter_by(
            project_id=project.id, resource_id=resource.id)
...
class TestNewHoldMain (CbankTester):
    ...
    def test_exists_and_callable (self):
        assert hasattr(cbank.cli.controllers, "new_hold_main"), \
            "new_hold_main does not exist"
        assert callable(cbank.cli.controllers.new_hold_main), \
...
class TestNewChargeMain (CbankTester):
    ...
    def test_exists_and_callable (self):
        assert hasattr(cbank.cli.controllers, "new_charge_main"), \
            "new_charge_main does not exist"
        assert callable(cbank.cli.controllers.new_charge_main), \
...
class TestNewRefundMain (CbankTester):
    ...
    def test_exists_and_callable (self):
        assert hasattr(cbank.cli.controllers, "new_refund_main"), \
            "new_refund_main does not exist"
        assert callable(cbank.cli.controllers.new_refund_main), \
    ...
    def test_without_charge (self):
        ...
        project = Project.fetch("project1")
        resource = Resource.fetch("resource1")
        refunds = lambda: Session.query(Refund)
        assert not refunds().count(), "started with existing refunds"
        allocation = Allocation(project=project, resource=resource,
            amount=1000, start=now-timedelta(days=1),
            end=now+timedelta(days=1))
...
class TestEditMain (CbankTester):
    ...
    def setup (self):
        CbankTester.setup(self)
        be_admin()
        self._edit_allocation_main = cbank.cli.controllers.edit_allocation_main
        self._edit_hold_main = cbank.cli.controllers.edit_hold_main
        self._edit_charge_main = cbank.cli.controllers.edit_charge_main
        self._edit_refund_main = cbank.cli.controllers.edit_refund_main
        cbank.cli.controllers.edit_allocation_main = FakeFunc()
        cbank.cli.controllers.edit_hold_main = FakeFunc()
        cbank.cli.controllers.edit_charge_main = FakeFunc()
        cbank.cli.controllers.edit_refund_main = FakeFunc()
    
    def teardown (self):
        CbankTester.teardown(self)
        cbank.cli.controllers.edit_allocation_main = self._edit_allocation_main
        cbank.cli.controllers.edit_hold_main = self._edit_hold_main
        cbank.cli.controllers.edit_charge_main = self._edit_charge_main
        cbank.cli.controllers.edit_refund_main = self._edit_refund_main
    
    def test_exists_and_callable (self):
        assert hasattr(cbank.cli.controllers, "edit_main"), "edit_main does not exist"
        assert callable(cbank.cli.controllers.edit_main), "edit_main is not callable"
    ...
    def test_allocation (self):
        ...
        def test_ ():
            ...
            assert sys.argv[1:] == args.split()[1:], sys.argv
        cbank.cli.controllers.edit_allocation_main.func = test_
        run(edit_main, args.split())
        assert cbank.cli.controllers.edit_allocation_main.calls
    ...
    def test_hold (self):
        ...
        def test_ ():
            ...
            assert sys.argv[1:] == args.split()[1:], sys.argv
        cbank.cli.controllers.edit_hold_main.func = test_
        run(edit_main, args.split())
        assert cbank.cli.controllers.edit_hold_main.calls
    ...
    def test_charge (self):
        ...
        def test_ ():
            ...
            assert sys.argv[1:] == args.split()[1:], sys.argv
        cbank.cli.controllers.edit_charge_main.func = test_
        run(edit_main, args.split())
        assert cbank.cli.controllers.edit_charge_main.calls
    ...
    def test_refund (self):
        ...
        def test_ ():
            ...
            assert sys.argv[1:] == args.split()[1:], sys.argv
        cbank.cli.controllers.edit_refund_main.func = test_
        run(edit_main, args.split())
        assert cbank.cli.controllers.edit_refund_main.calls
...
class TestEditAllocationMain (CbankTester):
    ...
    def setup (self):
        CbankTester.setup(self)
        be_admin()
        a = Allocation(Project.fetch("project1"), Resource.fetch("resource1"), 100,
    ...
    def test_exists_and_callable (self):
        assert hasattr(cbank.cli.controllers, "edit_allocation_main"), \
            "edit_allocation_main does not exist"
        assert callable(cbank.cli.controllers.edit_allocation_main), \
            "edit_allocation_main is not callable"
    
    def test_delete (self):
        args = "-D 1"
        code, stdout, stderr = run(
            cbank.cli.controllers.edit_allocation_main, args.split())
        assert_equal(code, 0)
    ...
    def test_no_commit (self):
        ...
        assert_equal(code, 0)
        a = Session.query(Allocation).filter_by(id=1).one()
        assert_identical(a.project, Project.fetch("project1"))
...
class TestEditHoldMain (CbankTester):
    ...
    def test_exists_and_callable (self):
        assert hasattr(cbank.cli.controllers, "edit_hold_main"), \
            "edit_hold_main does not exist"
        assert callable(cbank.cli.controllers.edit_hold_main), \
            "edit_hold_main is not callable"
    
    def test_comment (self):
        args = "-c newcomment 1"
        code, stdout, stderr = run(
            cbank.cli.controllers.edit_hold_main, args.split())
        Session.remove()
        assert_equal(code, 0)
...
class TestEditChargeMain (CbankTester):
    ...
    def setup (self):
        CbankTester.setup(self)
        be_admin()
        a1 = Allocation(Project.fetch("project1"), Resource.fetch("resource1"), 100,
            datetime(2008, 1, 1), datetime(2009, 1, 1))
        a2 = Allocation(Project.fetch("project1"), Resource.fetch("resource1"), 100,
            datetime(2008, 1, 1), datetime(2009, 1, 1))
        a1.id = 1
        a2.id = 2
        c = Charge(a1, 10)
    ...
    def test_delete (self):
        args = "-D 1"
        code, stdout, stderr = run(
            cbank.cli.controllers.edit_charge_main, args.split())
        assert_equal(code, 0)
    ...
    def test_exists_and_callable (self):
        assert hasattr(cbank.cli.controllers, "edit_charge_main"), \
            "edit_charge_main does not exist"
        assert callable(cbank.cli.controllers.edit_charge_main), \
...
class TestEditRefundMain (CbankTester):
    ...
    def test_exists_and_callable (self):
        assert hasattr(cbank.cli.controllers, "edit_refund_main"), \
            "edit_refund_main does not exist"
        assert callable(cbank.cli.controllers.edit_refund_main), \
            "edit_refund_main is not callable"
    
    def test_delete (self):
        args = "-D 1"
        code, stdout, stderr = run(
            cbank.cli.controllers.edit_refund_main, args.split())
        assert_equal(code, 0)
        assert_equal(list(Session.query(Refund).filter_by(id=1)), [])
...
class TestImportJobs (CbankTester):
    ...
    def test_job_from_pbs_s (self):
        ...
        job_ = jobs.one()
        assert_equal(job_.id, "692009.jmayor5.lcrc.anl.gov")
        assert_identical(job_.user, User.fetch("user1"))
        assert_equal(job_.group, "agroup")
        assert_identical(job_.account, Project.fetch("project1"))
    ...
    def test_job_from_pbs_e_with_charges (self):
        ...
        allocation = Allocation(Project.fetch("project1"), Resource.fetch("resource1"), 0,
            datetime(2008, 1, 1), datetime(2009, 1, 1))
    ...
    def test_job_from_pbs_e_update_run (self):
        ...
        job_ = jobs.one()
        assert_equal(job_.id, "691908.jmayor5.lcrc.anl.gov")
        assert_identical(job_.user, User.fetch("user2"))
        assert_identical(job_.account, Project.fetch("project2"))
...
class TestListMain (CbankTester):
    ...
    def setup (self):
        CbankTester.setup(self)
        self._list_users_main = cbank.cli.controllers.list_users_main
        self._list_projects_main = cbank.cli.controllers.list_projects_main
        self._list_allocations_main = cbank.cli.controllers.list_allocations_main
        self._list_holds_main = cbank.cli.controllers.list_holds_main
        self._list_jobs_main = cbank.cli.controllers.list_jobs_main
        self._list_charges_main = cbank.cli.controllers.list_charges_main
        cbank.cli.controllers.list_users_main = FakeFunc()
        cbank.cli.controllers.list_projects_main = FakeFunc()
        cbank.cli.controllers.list_allocations_main = FakeFunc()
        cbank.cli.controllers.list_holds_main = FakeFunc()
        cbank.cli.controllers.list_jobs_main = FakeFunc()
        cbank.cli.controllers.list_charges_main = FakeFunc()
    
    def teardown (self):
        CbankTester.teardown(self)
        cbank.cli.controllers.list_users_main = self._list_users_main
        cbank.cli.controllers.list_projects_main = self._list_projects_main
        cbank.cli.controllers.list_allocations_main = self._list_allocations_main
        cbank.cli.controllers.list_holds_main = self._list_holds_main
        cbank.cli.controllers.list_jobs_main = self._list_jobs_main
        cbank.cli.controllers.list_charges_main = self._list_charges_main
    
    def test_exists_and_callable (self):
        assert hasattr(cbank.cli.controllers, "list_main"), \
            "list_main does not exist"
        assert callable(cbank.cli.controllers.list_main), \
    ...
    def test_users (self):
        def test_ ():
            ...
            assert sys.argv[1:] == args.split()[1:], sys.argv
        cbank.cli.controllers.list_users_main.func = test_
        args = "users 1 2 3"
        run(list_main, args.split())
        assert cbank.cli.controllers.list_users_main.calls
    
    def test_projects (self):
        def test_ ():
            assert sys.argv[0] == "list_main projects", sys.argv
            assert sys.argv[1:] == args.split()[1:], sys.argv
        cbank.cli.controllers.list_projects_main.func = test_
        args = "projects 1 2 3"
        run(list_main, args.split())
        assert cbank.cli.controllers.list_projects_main.calls
    
    def test_allocations (self):
        def test_ ():
            assert sys.argv[0] == "list_main allocations", sys.argv
            assert sys.argv[1:] == args.split()[1:], sys.argv
        cbank.cli.controllers.list_allocations_main.func = test_
        args = "allocations 1 2 3"
        run(list_main, args.split())
        assert cbank.cli.controllers.list_allocations_main.calls
    
    def test_holds (self):
        def test_ ():
            assert sys.argv[0] == "list_main holds", sys.argv
            assert sys.argv[1:] == args.split()[1:], sys.argv
        cbank.cli.controllers.list_holds_main.func = test_
        args = "holds 1 2 3"
        run(list_main, args.split())
        assert cbank.cli.controllers.list_holds_main.calls
    
    def test_holds (self):
        def test_ ():
            assert sys.argv[0] == "list_main jobs", sys.argv
            assert sys.argv[1:] == args.split()[1:], sys.argv
        cbank.cli.controllers.list_jobs_main.func = test_
        args = "jobs 1 2 3"
        run(list_main, args.split())
        assert cbank.cli.controllers.list_jobs_main.calls
    
    def test_charges (self):
        def test_ ():
            assert sys.argv[0] == "list_main charges", sys.argv
            assert sys.argv[1:] == args.split()[1:], sys.argv
        cbank.cli.controllers.list_charges_main.func = test_
        args = "charges 1 2 3"
        run(list_main, args.split())
        assert cbank.cli.controllers.list_charges_main.calls
...
class TestUsersList (CbankTester):
    ...
    def setup (self):
        CbankTester.setup(self)
        self._print_users_list = cbank.cli.controllers.print_users_list
        cbank.cli.controllers.print_users_list = FakeFunc()
    
    def teardown (self):
        CbankTester.teardown(self)
        cbank.cli.controllers.print_users_list = self._print_users_list
    ...
    def test_default (self):
        ...
        assert_equal(code, 0)
        args, kwargs = cbank.cli.controllers.print_users_list.calls[0]
    ...
    def test_other_users (self):
        ...
        assert_equal(code, NotPermitted.exit_code)
        assert not cbank.cli.controllers.print_users_list.calls
    ...
    def test_project_admin_projects (self):
        project = Project.fetch("project3")
...
class TestProjectsList (CbankTester):
    ...
    def setup (self):
        CbankTester.setup(self)
        self._print_projects_list = cbank.cli.controllers.print_projects_list
        cbank.cli.controllers.print_projects_list = FakeFunc()
    
    def teardown (self):
        CbankTester.teardown(self)
        cbank.cli.controllers.print_projects_list = self._print_projects_list
    ...
    def test_default (self):
        ...
        assert_equal(code, 0)
        args, kwargs = cbank.cli.controllers.print_projects_list.calls[0]
        assert_equal(set(args[0]), set(projects))
    
    def test_member_projects (self):
        """a specific project of the user"""
        Session.add(Allocation(
            Project.fetch("project2"), Resource.fetch("resource1"),
    ...
    def test_project_admin_projects (self):
        ...
        Session.add(Allocation(
            Project.fetch("project3"), Resource.fetch("resource1"),
            0, datetime(2000, 1, 1), datetime(2000, 1, 1)))
    ...
    def test_other_projects (self):
        ...
        assert_equal(code, NotPermitted.exit_code)
        assert not cbank.cli.controllers.print_projects_list.calls
...
def projects (users):
    return sum((get_projects(user) for user in users), [])


def allocations (projects):
    return Session.query(Allocation).filter(Allocation.project_id.in_(project.id for project in projects))
...
class TestAllocationsList (CbankTester):
    ...
    def teardown (self):
        CbankTester.teardown(self)
        cbank.cli.controllers.print_allocations_list = self._print_allocations_list
    ...
    def test_default (self):
        ...
        assert_equal(code, 0)
        args, kwargs = cbank.cli.controllers.print_allocations_list.calls[0]
    ...
    def test_other_projects (self):
        ...
        assert_equal(code, NotPermitted.exit_code)
        assert not cbank.cli.controllers.print_allocations_list.calls
    ...
    def test_self_users (self):
        user = current_user()
        projects = get_projects(member=user)
        code, stdout, stderr = run(
    ...
    def test_resources (self):
        allocations_ = active([allocation
            for allocation in allocations(get_projects(member=current_user()))
    ...
    def test_after (self):
        user = current_user()
        projects = get_projects(member=user)
        allocations_ = allocations(projects)
...
class TestHoldsList (CbankTester):
    ...
    def setup (self):
        CbankTester.setup(self)
        self._print_holds_list = cbank.cli.controllers.print_holds_list
        cbank.cli.controllers.print_holds_list = FakeFunc()
        user1, user2 = [User.fetch(user) for user in ["user1", "user2"]]
        for project in [Project.cached(id_) for id_ in ["1", "2", "3", "4"]]:
            Session.add_all([
                Allocation(
                    project, Resource.fetch("resource1"), 0,
                    datetime(2000, 1, 1), datetime(2001, 1, 1)),
                Allocation(
                    project, Resource.fetch("resource2"), 0,
                    datetime(2000, 1, 1), datetime(2001, 1, 1))])
    ...
    def teardown (self):
        CbankTester.teardown(self)
        cbank.cli.controllers.print_holds_list = self._print_holds_list
    
    def test_default (self):
        holds = Session.query(Hold).filter_by(active=True)
        holds = holds.filter(Hold.job.has(user_id=current_user().id))
        holds = holds.filter(Hold.allocation.has(
            Allocation.project_id.in_(project.id for project in
            get_projects(current_user()))))
        code, stdout, stderr = run(list_holds_main)
        assert_equal(code, 0)
        args, kwargs = cbank.cli.controllers.print_holds_list.calls[0]
    ...
    def test_other_users (self):
        ...
        assert_equal(code, NotPermitted.exit_code)
        assert not cbank.cli.controllers.print_holds_list.calls
    ...
    def test_project_admin_users (self):
        ...
        holds = holds.filter(Hold.job.has(user_id=User.fetch("user1").id))
        code, stdout, stderr = run(list_holds_main,
    ...
    def test_self_users (self):
        ...
        holds = Session.query(Hold).filter_by(active=True)
        holds = holds.filter(Hold.allocation.has(
            Allocation.project_id.in_(project.id
    ...
    def test_resources (self):
        ...
        holds = holds.filter(Hold.allocation.has(
            Allocation.project_id.in_(project.id for project in
            get_projects(current_user())))).filter(Hold.allocation.has(
    ...
    def test_after (self):
        ...
        holds = holds.filter(Hold.job.has(user_id=current_user().id))
        holds = holds.filter(Hold.allocation.has(
            Allocation.project_id.in_(project.id for project in get_projects(current_user())))).filter(
    ...
    def test_before (self):
        ...
        holds = holds.filter(Hold.allocation.has(
            Allocation.project_id.in_(project.id for project in
            get_projects(current_user())))).filter(
            Hold.datetime < datetime(2000, 1, 1))
...
class TestJobsList (CbankTester):
    ...
    def setup (self):
        CbankTester.setup(self)
        self._print_jobs_list = cbank.cli.controllers.print_jobs_list
        cbank.cli.controllers.print_jobs_list = FakeFunc()
        current_user_ = current_user()
        p1r1 = Allocation(Project.fetch("project1"), Resource.fetch("resource1"), 0,
            datetime(2000, 1, 1), datetime(2001, 1, 1))
        p2r1 = Allocation(Project.fetch("project2"), Resource.fetch("resource1"), 0,
            datetime(2000, 1, 1), datetime(2001, 1, 1))
        p4r1 = Allocation(Project.fetch("project4"), Resource.fetch("resource1"), 0,
            datetime(2000, 1, 1), datetime(2001, 1, 1))
        p2r2 = Allocation(Project.fetch("project2"), Resource.fetch("resource2"), 0,
            datetime(2000, 1, 1), datetime(2001, 1, 1))
        j1 = Job("resource1.1")
        j1.user = current_user_
        j1.start = datetime(2000, 1, 1)
        j1.end = datetime(2000, 1, 2)
        j1.account = Project.fetch("project2")
        ...
        j5 = Job("resource1.5")
        j5.user = User.fetch("user1")
        j5.start = datetime(2000, 1, 30)
        j5.end = datetime(2000, 2, 2)
        j5.account = Project.fetch("project4")
    ...
    def teardown (self):
        CbankTester.teardown(self)
        cbank.cli.controllers.print_jobs_list = self._print_jobs_list
    
    def test_default (self):
        code, stdout, stderr = run(list_jobs_main)
        assert_equal(code, 0)
        args, kwargs = cbank.cli.controllers.print_jobs_list.calls[0]
    ...
    def test_other_users (self):
        code, stdout, stderr = run(list_jobs_main,
            "-p project1 -u user1".split())
        assert_equal(code, NotPermitted.exit_code)
        assert not cbank.cli.controllers.print_jobs_list.calls
...
class TestChargesList (CbankTester):
    ...
    def setup (self):
        CbankTester.setup(self)
        self._print_charges_list = cbank.cli.controllers.print_charges_list
        cbank.cli.controllers.print_charges_list = FakeFunc()
        user1, user2 = [User.fetch(user) for user in ["user1", "user2"]]
        for project in [Project.cached(id_) for id_ in ["1", "2", "3", "4"]]:
            Session.add_all([
                Allocation(
    ...
    def teardown (self):
        CbankTester.teardown(self)
        cbank.cli.controllers.print_charges_list = self._print_charges_list
    ...
    def test_default (self):
        ...
        assert_equal(code, 0)
        args, kwargs = cbank.cli.controllers.print_charges_list.calls[0]
    ...
    def test_other_users (self):
        code, stdout, stderr = run(list_charges_main,
            "-p project1 -u user1".split())
        assert_equal(code, NotPermitted.exit_code)
        assert not cbank.cli.controllers.print_charges_list.calls
...
class TestDetailJobs (CbankTester):
    ...
    def setup (self):
        ...
        Session.add(job_)
        self._print_jobs = cbank.cli.controllers.print_jobs
        cbank.cli.controllers.print_jobs = FakeFunc()
    
    def teardown (self):
        CbankTester.teardown(self)
        cbank.cli.controllers.print_jobs = self._print_jobs
    ...
    def test_jobs_list (self):
        ...
        assert_equal(code, 0)
        jobs = Session.query(Job).filter(Job.id.in_(["resource1.1"]))
        args, kwargs = cbank.cli.controllers.print_jobs.calls[0]
        assert_equal(set(args[0]), set(jobs))
